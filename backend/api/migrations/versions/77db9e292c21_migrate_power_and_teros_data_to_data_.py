...
def upgrade():
    ...
    cell_ids_query = sa.select([
        sa.distinct(power_data.c.cell_id)
    ])
    ...
    for cell_id in cell_ids:
        ...
        if not voltage_sensor_id:
            # Get the logger_id (use the first one found for this cell_id)
            logger_id_query = sa.select([power_data.c.logger_id]).where(
                power_data.c.cell_id == cell_id
            ).limit(1)
            logger_id = connection.execute(logger_id_query).scalar()
            ...
            voltage_sensor_id = connection.execute(voltage_sensor_insert).inserted_primary_key[0]
        ...
        if not current_sensor_id:
            ...
            current_sensor_id = connection.execute(current_sensor_insert).inserted_primary_key[0]
        ...
        while True:
            ...
            print(f"Migrating {len(power_records)} voltage records for cell_id {cell_id}")
    ...
    cell_ids_query = sa.select([
        sa.distinct(teros_data.c.cell_id)
    ])
    cell_ids = [row[0] for row in connection.execute(cell_ids_query)]
    ...
    for cell_id in cell_ids:
        ...
        if not vwc_sensor_id:
            ...
            vwc_sensor_id = connection.execute(vwc_sensor_insert).inserted_primary_key[0]
        ...
        if not raw_vwc_sensor_id:
            ...
            raw_vwc_sensor_id = connection.execute(raw_vwc_sensor_insert).inserted_primary_key[0]
        ...
        if not temp_sensor_id:
            ...
            temp_sensor_id = connection.execute(temp_sensor_insert).inserted_primary_key[0]
        ...
        if not ec_sensor_id:
            ...
            ec_sensor_id = connection.execute(ec_sensor_insert).inserted_primary_key[0]
    ...
    op.execute("""
    COMMENT ON TABLE data IS 'Power and TEROS data migrated from legacy tables on 2025-05-16'
    """)
