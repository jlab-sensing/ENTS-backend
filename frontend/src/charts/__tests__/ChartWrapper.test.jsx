...
describe('testing side button events', () => {
  ...
  it('should open downsample menu when downsample button is clicked', async () => {
    const user = userEvent.setup();
    render(
      <MockChartWrapper
        id='vwc'
    ...
    expect(noneMenuItem).toBeInTheDocument();
    expect(hourlyMenuItem).toBeInTheDocument();
    expect(dailyMenuItem).toBeInTheDocument();
  });
  ...
  it('should select daily resample option', async () => {
    const user = userEvent.setup();
    const mockOnResampleChange = vi.fn();
  ...
  it('should select none resample option', async () => {
    const user = userEvent.setup();
    const mockOnResampleChange = vi.fn();
  ...
  it('should close fullscreen modal when minimize button is clicked', async () => {
    const user = userEvent.setup();
    render(
      <MockChartWrapper
        id='vwc'
    ...
    const minimizeBtnElement = await screen.findByLabelText(/Windowed/i);
    await user.click(minimizeBtnElement);

    await vi.waitFor(() => {
      fullscreenModalElement = screen.queryByTestId(/^fullscreen-modal$/);
      expect(fullscreenModalElement).not.toBeInTheDocument();
    });
  });
});

describe('streaming mode tests', () => {
  it('should not show zoom/pan controls in streaming mode', () => {
    render(
      <MockChartWrapper
        id='vwc'
        data={data}
    ...
    const zoomBtn = screen.queryByLabelText(/^Zoom$/);
    const panBtn = screen.queryByLabelText(/^Pan$/);
    const zoomInBtn = screen.queryByLabelText(/Zoom In/i);
    const zoomOutBtn = screen.queryByLabelText(/Zoom Out/i);
    const downsampleBtn = screen.queryByLabelText(/Downsample/i);
    const exportBtn = screen.queryByLabelText(/Export Chart/i);

    expect(zoomBtn).not.toBeInTheDocument();
    expect(panBtn).not.toBeInTheDocument();
    expect(zoomInBtn).not.toBeInTheDocument();
    expect(zoomOutBtn).not.toBeInTheDocument();
    expect(downsampleBtn).not.toBeInTheDocument();
    expect(exportBtn).not.toBeInTheDocument();
  });

  it('should show reset and fullscreen buttons in streaming mode', async () => {
    render(
      <MockChartWrapper
        id='vwc'
        data={data}
    ...
    const resetBtn = await screen.findByLabelText(/Reset/i);
    const fullscreenBtn = await screen.findByLabelText(/Fullscreen/i);
